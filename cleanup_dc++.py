...
def parse_xml_entry(entry_element) -> FileEntry:
    """
    Parse a single XML file entry and return a FileEntry object
    
    Example entry:
    <File Name="Champions 001 (2019) (Digital) (Zone-Empire).cbr" 
          Size="36904422" 
          TTH="RVPDAATGGUMOTJWDJCF7VTIA3UNTJA42YIUQW5Y"/>
    """
    name = entry_element.get('Name')
    size = int(entry_element.get('Size'))
    tth = entry_element.get('TTH')
    
    return FileEntry(name, size, tth)

def load_xml_file(xml_path: str) -> List[FileEntry]:
    """Load and parse an XML file containing file entries"""
    try:
        ...
        for entry in root.findall('.//File'):
            entries.append(parse_xml_entry(entry))
...
def compare_xml_files(mine_path: str, first_path: str, second_path: str, third_path: str) -> Dict[str, List[FileEntry]]:
    """
    Compare four XML files and categorize entries based on TTH matches
    """
    # Load all XML files
    mine_entries = {entry.tth: entry for entry in load_xml_file(mine_path)}
    first_entries = {entry.tth: entry for entry in load_xml_file(first_path)}
    second_entries = {entry.tth: entry for entry in load_xml_file(second_path)}
    third_entries = {entry.tth: entry for entry in load_xml_file(third_path)}
...
def main():
    ...
    mine_path = "mine.xml"
    first_path = "bigfirst.xml"
    second_path = "bigsecond.xml"
    third_path = "bigthird.xml"
    ...
    try:
        comparison = compare_xml_files(mine_path, first_path, second_path, third_path)
